...
function injectCase(code, eventType, caseStr) {
    ...
    // Quick check if the case name is already in the switch block
    let caseNameMatch = caseStr.match(/case '([^']+)':/);
    if (caseNameMatch && code.slice(switchIndex, switchIndex + 2000).includes(caseNameMatch[0])) {
        return code; // skip
    }
...
console.log('Delegator modified safely.');
