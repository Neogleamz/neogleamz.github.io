...
- **The Anti-Hallucination Legacy Ban:** You are STRICTLY FORBIDDEN from ever generating or injecting `innerHTML` assignments without wrapping the payload in `window.safeHTML()`. You are STRICTLY FORBIDDEN from generating inline DOM event handlers (`onclick=`, `onmouseover=`, `onchange=`). All dynamic UI interactions MUST use `data-click` tokens bound to the `system-event-delegator.js`.
