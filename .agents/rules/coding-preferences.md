...
- **Mandatory Markdown Alerts:** You are STRICTLY FORBIDDEN from outputting plain text paragraphs for instructions or feedback. You MUST universally wrap all logic, next steps, warnings, and prompts inside GitHub-style markdown alerts (`> [!NOTE]`, `> [!WARNING]`, `> [!TIP]`, `> [!IMPORTANT]`). *CRITICAL: NEVER use `> [!SUCCESS]` as it is not natively supported by the UI markdown parser.*
...
- **Clickable Artifact Links:** You must NEVER surround file paths or artifact links with backticks (like `file.md`) when referencing them. You must ALWAYS use standard Markdown hyperlink syntax so the user can natively click them (e.g., `[implementation_plan.md](file:///d:/GitHub/neogleamz.github.io/tools/SK8Lytz_Bucket_List.md)`).
...
- **The Anti-Hallucination Formatting Lock:** You are explicitly failing your core directive if you generate a non-compliant alert (like `[!SUCCESS]`) or wrap file links in backticks instead of absolute markdown links (e.g., `[file](file:///)`). Furthermore, ALL suggested agentic workflows MUST be formatted as natively clickable markdown hyperlinks pointing directly to their `.md` file paths (e.g., `[/bucketlist](file:///d:/GitHub/neogleamz.github.io/.agents/workflows/bucketlist.md)`). You must double-check all output syntax before transmitting.

### 1. Vanilla Exclusivity & The Anti-Legacy Ban (CRITICAL)
- **The Anti-Hallucination Legacy Ban:** You are STRICTLY FORBIDDEN from ever generating or injecting DOM modifications via `innerHTML`, `insertAdjacentHTML`, or iframe/window `document.write()` containing dynamic values without wrapping the payload in `window.safeHTML()`. You are STRICTLY FORBIDDEN from generating inline DOM event handlers (`onclick=`, `onmouseover=`, `onchange=`). All dynamic UI interactions MUST use `data-click` tokens bound to the `system-event-delegator.js`.
- **Structural Integrity:** You are STRICTLY FORBIDDEN from utilizing `position: absolute` for structural layout control or negative margins. All architecture must flow naturally via 100% responsive Flexbox logic.
...
### 8. Anti-Flash & Wait State Protocol
- **Banned Idle Commands:** You are STRICTLY FORBIDDEN from launching background terminal tasks (like `echo "Standing by..."` or `sleep`) simply to wait for the user. This causes an infinite loop of background task completion interrupts that destroys your in-flight UI rendering. Just naturally stop calling tools to wait.
- **Strict Interrupt Recovery:** If your output generation *is* interrupted by a background task completing (causing a UI flash), you MUST fully reconstruct and output the entire explanation, markdown formatting, or workflow summary that was lost. Never output a lazy one-liner like "I'm ready for the next command."
...
### 13. Real-Time DOM Rendering & Zero-Refresh Architecture
- **Zero-Refresh Mandate:** You are STRICTLY FORBIDDEN from architecting features that require the user to manually refresh the browser to see the result of their actions (e.g., inventory consumption, database mutations, or state changes).
- **Synchronous View Updates:** Whenever an asynchronous backend process successfully resolves (like a Supabase upsert or print deduction), you MUST immediately invoke the corresponding DOM rendering functions (e.g., `renderInventoryTable()`, `renderBarcodzGrid()`, `renderLabelzGrid()`) in the `.then()` or `await` success block to instantly paint the new state.
- **Cross-View Propagation:** You must consider the holistic state of the application. If a mutation in one component (e.g., Print Spooler) affects data natively displayed in another component (e.g., the Labelz Grid or Stockpilez table), you must ensure the render cycles for ALL affected views are triggered simultaneously without user intervention.
