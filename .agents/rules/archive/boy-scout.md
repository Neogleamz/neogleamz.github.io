...
5. **EXEMPTION (The Collision Rule)**: You must entirely suspend the Boy Scout rule when executing bug fixes (on `fix/` branches or hotfixes), executing an `Isolated Test` (where scope is locked to a Git diff), or performing a `Surgical Strike` on a highly complex monolithic file where touching unrelated lines is strictly banned. The Boy Scout sweep is exclusively reserved for new feature builds (`feat/`) or broad refactoring (`refactor/`).
