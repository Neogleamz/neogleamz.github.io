...
When my prompt includes "whats up", "status update", "where are we", "what's happening", or "current status", you must act as the Project Manager and execute the following workflow to generate a Situation Report (SITREP):
...
2. **Parse the Bucket List**: Read `.agents/workflows/bucket_list.md`.
