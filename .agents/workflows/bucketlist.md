...
1. **Read Status**: Parse the `.agents/workflows/bucket_list.md` file.
...
- Ensure the artifact's `request_feedback` flag is set to `true`.
  - Additionally, save a physical copy of this plan to the codebase at `.agents/workflows/plans/<extracted-branch-slug>.md`.
  - Use the `run_command` tool to execute `cat .agents/workflows/plans/<extracted-branch-slug>.md` to display  the plan in the terminal log. Do NOT print the plan raw text into the chat.
...
2. **Update Tracking**: Modify `.agents/workflows/bucket_list.md` using your editing tools and change the checkbox for this item to `- [x]`.

3. **Display Status and Halt**: Use the `run_command` tool to execute `cat .agents/workflows/bucket_list.md` to print the updated status to the terminal log. Do NOT print the raw contents to the chat. Do not take any further action. Wait for me to explicitly tell you to begin the next item.
