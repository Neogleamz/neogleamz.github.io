...
### 🔒 Security
- `fix/print-reorder-report-document-write-xss` : **Unsanitized document.write — Print Reports** — Closed a stored-XSS regression in the Low Stockz Report print window (a previously-archived fix for this exact function had silently regressed since 2026-07-01), plus an identical, previously-undocumented instance in the KPI Report print window discovered during planning. Malicious catalog/product names or specs could execute arbitrary JavaScript in the print popup. Both now sanitize the assembled HTML via `DOMPurify.sanitize()` immediately before `document.write`, matching the established pattern already correct in 3 sibling print-window functions elsewhere in the app. Verified: 0 new XSS violations (confirmed via manual exploit reproduction — this bug class is a documented scanner blind spot), 59/59 tests, 0 lint errors/warnings.
...
### 🧹 Chores & Cleanup
- `debt/nomenclature-remediation` : **Nomenclature Remediation — Batched Tier 1 & Tier 2** — Triaged all 7 nomenclature-audit categories across 11 batches. Closed N2_ORPHAN_HANDLER (35→0), N3_LABEL_DRIFT (1→0), N5_NEW_NONCONFORMANT_KEY (24→0), N6_UNUSED_CSS (90→29 confirmed live + intentional), N7_DICT_STALE (0). Documented permanent floors: N1_GHOST_ID (1 deliberately unguarded, tied to separate bug), N4_LEGACY_TERM (84 protected aliases, gated to Phase 4). Discovered and deferred 5 real live bugs for fix-specific tasks; caught and corrected multiple explore-mapper hallucinations; established precedents for blast-radius verification and topological-integrity audits. Epic scope was full triage with evidence, not absolute-zero pursuit. Verified: 0 XSS violations, 59/59 tests, 0 lint errors/warnings.
...
- `debt/brand-sweep` : **Brand Sweep & Hook Flip to Blocking — Closed Without Implementation** — Scoped via a full `/bucketlist` pre-task swarm before any code was touched; user declined both the internal neogleamz→sk8lytz identifier rename sweep and the `.githooks/pre-commit` flip to blocking, since the task's real original intent (tab/page label accuracy) was already fully satisfied by `debt/nomenclature-remediation` Batch 11. No files changed — `.githooks/pre-commit` remains advisory (`--warn`).
- `debt/master-reference-doc-sync` : **Master Reference Doc Sync** — Corrected 5 stale documentation references (`tools/SK8Lytz_App_Master_Reference.md` ×4, `docs/ARCHITECTURE.md` ×1) that described a retired DOM id (`cycleCountManagerModal`/`#barcode-reader`) as the live STOCKZ scanner UI. Updated to cite the live `stockzAuditModal`/`stockzAuditLocalReader` ids while preserving the iOS Safari aspect-ratio hardware-constraint knowledge and noting the still-live `openCycleCountManager`/`closeCycleCountManager` compatibility aliases. Docs-only, zero application code changed. Verified: 0 XSS violations, 59/59 tests, 0 lint errors/warnings.

## [1.5.3] - 2026-07-17

### ✨ Features & Bug Fixes
- `fix/dead-ui-wiring` : **Production UI Wiring — Dead Buttons & Crashes** — Wired 3 dead `data-click` tokens to their existing handler functions in `system-event-delegator.js` (`click_sortLtvModal` ×6 headers, `click_actualNetSort_a`, `click_cancelRestore`) whose DOM emitters existed but had no dispatch case. Removed a duplicate `ceo-module.js` click listener that would have double-fired `sortLtvModal()` once the delegator case was added, fixing a "stuck" sort-toggle regression. Null-guarded 7 `getElementById` calls in `inventory-module.js`/`packerz-module.js` that previously threw `TypeError` if their target elements were absent. Phase 0 of the Nomenclature Audit Engine epic, shipped standalone. Verified: 0 XSS violations, 59/59 tests, 0 lint errors/warnings.
- `feat/nomenclature-registry` : **Nomenclature Registry** — Added `tools/nomenclature-registry.json` as the single machine-readable naming authority (6 hubs, 9 legacy pane-id aliases, 3 function aliases, 1 delegator-token alias, 7 rename-forbidden identifier families, 8 dynamic-id allowlist patterns) plus `scripts/generate-nomenclature-dictionary.js` to deterministically regenerate `docs/nomenclature_dictionary.md` from it, replacing a hand-maintained copy that had already drifted from the Master Reference. Recon also caught and fixed two pre-existing doc-only bugs: both the Master Reference and the old dictionary listed fictional "legacy" hub-tab DOM ids (`invhub-tab`, `prodhub-tab`, etc.) that never existed in code, and Master Reference §6B cited 3 non-existent NEXUZ pane ids. Phase 1 of the Nomenclature Audit Engine epic. Verified: 0 XSS violations, 59/59 tests, 0 lint errors/warnings.
- `feat/nomenclature-audit-engine` : **Nomenclature Audit Engine & Pre-Commit Integration** — Added `scripts/nomenclature-audit.js`, a two-pass scanner implementing all 7 ADR checks (N1 ghost DOM lookups, N2 orphan/dead delegator tokens, N3 label drift, N4 legacy-term occurrences, N5 localStorage key conformance, N6 unused CSS, N7 registry↔dictionary sync) against the Phase 1 registry, wired into `.githooks/pre-commit` in advisory-only mode alongside the existing blocking XSS gate. Initial baseline: 358 findings (216 deduped fingerprints) — N2 came back 0/0 (Phase 0 already resolved delegator orphans), N1 ghost lookups is the dominant real category. Phase 2 of the Nomenclature Audit Engine epic. Verified: 0 XSS violations, 59/59 tests, 0 lint errors/warnings.
