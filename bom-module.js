...
function openBulkAddModal() {
    ...
    Object.keys(productsDB).sort().forEach(p => { if(p === currentProduct) return; bulkAddData.push({ k: `RECIPE:::${p}`, isSub: true, nn: isSubassemblyDB[p] ? "⚙️" : "📦", np: "Sub-Assembly", n: p, sp: "(Nested Recipe)", uc: calculateProductTotal(p), q: "" }); });
...
async function createNewProduct() { try { let n = prompt("Recipe Name:"); if(!n || !n.trim() || productsDB[n.trim()]) return; n = n.trim(); productsDB[n] = []; laborDB[n] = {time:0, rate:0}; pricingDB[n] = {msrp:0, wholesale:0}; isSubassemblyDB[n] = false; await supabaseClient.from('product_recipes').insert({product_name: n, components: [], labor_time_mins: 0, labor_rate_hr: 0, msrp: 0, wholesale_price: 0, is_subassembly: false}); currentProduct = n; renderProductList(); renderProductBOM(); if(typeof populateDropdowns === 'function') populateDropdowns(); } catch(e) { sysLog(e.message, true); } }
async function deleteCurrentProduct() { try { if(!currentProduct) return; if(confirm(`Delete ${currentProduct}?`)){ sysLog(`Deleting ${currentProduct}`); await supabaseClient.from('product_recipes').delete().eq('product_name', currentProduct); delete productsDB[currentProduct]; delete laborDB[currentProduct]; delete pricingDB[currentProduct]; delete isSubassemblyDB[currentProduct]; let ups = []; Object.keys(productsDB).forEach(n => { let oL = productsDB[n].length; productsDB[n] = productsDB[n].filter(x => String(x.item_key || x.di_item_id || x.name) !== 'RECIPE:::'+currentProduct); if(productsDB[n].length !== oL) ups.push(supabaseClient.from('product_recipes').update({components: productsDB[n]}).eq('product_name', n)); }); if(ups.length>0) await Promise.all(ups); currentProduct = Object.keys(productsDB)[0]||null; if(typeof populateDropdowns === 'function') populateDropdowns(); renderProductList(); } } catch(e) { sysLog(e.message, true); } }
async function nukeAllRecipes() { try { if(confirm("⚠️ DANGER: Delete ALL recipes?")){ sysLog("Nuking recipes..."); setMasterStatus("Nuking...", "mod-working"); const {error}=await supabaseClient.from('product_recipes').delete().neq('product_name','fake_key'); if(error) throw new Error(error.message); productsDB={}; laborDB={}; pricingDB={}; isSubassemblyDB={}; currentProduct=null; renderProductList(); if(typeof populateDropdowns === 'function') populateDropdowns(); document.getElementById('bomMainArea').style.display='none'; setMasterStatus("Nuked.", "mod-success"); setTimeout(()=>setMasterStatus("Ready.", "status-idle"),3000); } } catch(e) { sysLog(e.message, true); setMasterStatus("Fail.", "mod-error"); } }
