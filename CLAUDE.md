...
**Named subagents** live in `.claude/agents/` and each carries a pinned model — dispatch them by `subagent_type` and do NOT pass a `model` override unless escalating an `implementer` to `opus` for a security-critical file: `explore-mapper` (haiku), `test-lint-runner` (haiku), `security-scout` (sonnet), `implementation-planner` (sonnet), `xss-validator` (sonnet), `test-guide-generator` (sonnet), `implementer` (sonnet), `checkpoint-agent` (haiku), `git-cleanup-agent` (haiku), `status-reporter` (haiku), `silent-release-agent` (haiku), `emergency-pivot-agent` (haiku), `diff-reviewer` (sonnet).

**Full subagent-shell conversions (real tier enforcement, not just documentation):** a handful of commands are fully deterministic and needs no prior conversation context, so they immediately dispatch their entire task to a named subagent instead of running inline — this is the only way a command's tier is actually guaranteed regardless of session model: `/gitcleanup` → `git-cleanup-agent`, `/status-update` → `status-reporter`, `/silent-release` → `silent-release-agent`, `/save-point` Path A (Checkpoint) → `checkpoint-agent`, `/intake` steps 1-4 (mechanical prep) → `emergency-pivot-agent`. Do NOT convert commands that need live conversation context (`/idea_intake`, `/jargon_brake`, `/rubber_duck`, `/echo_protocol` — a cold subagent can't see what was just discussed) or that have a mid-task wait-for-user-input gate a single dispatch can't pause for (`/save-point` Path B, `/finalize_epic`, `/schema_diff`, `/intake` step 5, and every sonnet/opus-tier gate-driven workflow).
