...
window.renderBulkAddBody = function() {
    ...
    filtered.forEach(x => { let sk = String(x.k).replace(/'/g, "\\'").replace(/"/g, '&quot;'); let displaySpec = x.sp === "(Mixed Specs)" ? "⚙️ (Mixed Specs)" : x.sp; if(x.isSub) { let safeSubName = String(x.k).replace('RECIPE:::', '').replace(/'/g, "\\'").replace(/"/g, '&quot;'); h += `<tr><td tabindex="0" class="trunc-col" style="font-weight:bold; color:var(--accent-color, #38bdf8); cursor:pointer; text-decoration:underline;" data-app-click="selectProd" data-name="${safeSubName}" data-click="click_document_getElementById_bulkAd">${x.nn}</td><td tabindex="0" class="trunc-col" style="color:var(--text-muted);">${x.np}</td><td tabindex="0" class="trunc-col">${x.n}</td><td tabindex="0" class="trunc-col">(Nested Recipe)</td><td class="text-right">$${x.uc.toFixed(4)}</td><td style="text-align:center;"><input type="number" class="bulk-qty-input" value="${x.q}" data-app-input="updateBulkQty" data-key="${sk}" min="0" step="any" placeholder="0" style="width:80px;text-align:center;"></td></tr>`; } else { h += `<tr><td tabindex="0" class="trunc-col" style="font-weight:bold;color:var(--text-heading);">${x.nn}</td><td tabindex="0" class="trunc-col" style="color:var(--text-muted);">${x.np}</td><td tabindex="0" class="trunc-col">${x.n}</td><td tabindex="0" class="trunc-col">${displaySpec}</td><td class="text-right">$${x.uc.toFixed(4)}</td><td style="text-align:center;"><input type="number" class="bulk-qty-input" value="${x.q}" data-app-input="updateBulkQty" data-key="${sk}" min="0" step="any" placeholder="0" style="width:80px;text-align:center;"></td></tr>`; } });
    wrap.innerHTML = window.safeHTML(h + `</tbody></table>`); applyTableInteractivity('bulkAddTableWrap');
}
...
window.renderProductList = function() {
    ...
    if(allProds.length===0){ ui.innerHTML = window.safeHTML("<li style='cursor:default; background:transparent; border:none; color:var(--text-main);'>No products.</li>"); document.getElementById('bomMainArea').style.display='none'; return; }
...
window.renderProductBOM = function() {
    ...
    wrap.innerHTML = window.safeHTML(h + `</tbody></table>`); document.getElementById('bomTotalCost').innerText = `$${getEngineTrueCogs(currentProduct).toFixed(2)}`; applyTableInteractivity('bomTableWrap');
}
...
window.updateRecipeManagerStaging = function(el) {
    ...
    if (idx >= 0 && idx < window.recipeManagerStaging.length) {
        ...
        if (tr) {
            ...
            statusTd.innerHTML = window.safeHTML(payload);
            }
        }
    }
};
