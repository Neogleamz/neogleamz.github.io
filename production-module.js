                        ...
                        if(s.isHeader) {
                            ...
                            [s.m1, s.m2, s.m3].forEach(m => {
                                if(m && m.url) {
                                    ...
                                    else { let mediaUrl = dId ? `https://drive.google.com/file/d/${dId}/preview` : safeUrl; let icon = m.type === 'vid' ? '🎬' : '📄'; attachmentHtml += `<div style="position:relative; width: 300px; height: 150px; background:var(--bg-glass); border-radius: 6px; border: 1px dashed var(--border-color); cursor: zoom-in; display:flex; flex-direction:column; align-items:center; justify-content:center; color:var(--text-muted); transition:all 0.2s;" onmouseover="this.style.background='rgba(59,130,246,0.1)';this.style.borderColor='#3b82f6';this.style.color='#3b82f6';" onmouseout="this.style.background='var(--bg-glass)';this.style.borderColor='var(--border-color)';this.style.color='var(--text-muted)';" onclick="openMediaModal('${mediaUrl}', 'iframe')"><div style="font-size:32px; margin-bottom:10px;">${icon}</div><div style="font-size:12px; font-weight:bold;">Load External Media</div></div>`; }
                                }
                            });
                            attachmentHtml += `</div>`;
