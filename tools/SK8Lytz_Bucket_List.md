...
- [ ] `debt/security` : **[HIGH]** `xlsx` (SheetJS) CVEs Prototype Pollution + ReDoS. While we migrated to `exceljs` via CDN previously, `xlsx` is still physically defined in `package.json` dependencies and failing the security audit. It needs to be `npm uninstall xlsx` securely.
