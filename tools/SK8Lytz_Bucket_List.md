...
- [x] `refactor/var-to-const-let` : **Legacy var → let/const Upgrade** - Systematically upgrade all legacy `var` declarations to block-scoped `let`/`const` across all 16 production modules, verifying no hoisting-dependent logic breaks. [🤖 Gemini 3.1 Pro] [🧠 5k / 20k] [💸 $0.02 / $0.06]
- [x] `refactor/event-listener-cleanup` : **Event Listener Memory Leak Audit** - Add `removeEventListener` cleanup to the 10 modules missing it (task-engine, sales, scraper, orders, labelz, inventory, ceo, bom, barcodz, analytics) to prevent memory leaks on view re-renders. [🤖 Gemini 3.1 Pro] [🧠 15k / 15k] [💸 $0.05 / $0.05]
