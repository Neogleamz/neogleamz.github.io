...
### Target: `main`
**Epic: Security — Unsafe insertAdjacentHTML Fallback**
*(Archived — 2026-06-30)*
- [x] `debt/security` : **[packerz-module.js:1581](../assets/js/packerz-module.js)** — `insertAdjacentHTML('beforeend', window.safeHTML ? window.safeHTML(h) : h)` — the ternary fallback `: h` injects raw unguarded HTML if `window.safeHTML` is undefined. Hardened to early-return with console error if safeHTML unavailable.
