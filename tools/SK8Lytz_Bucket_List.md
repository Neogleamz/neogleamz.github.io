...
- [x] `feat/task-engine-sorting` : **Task Sorting & Prioritization** - Implement the ability to sort tasks globally and prioritize them (e.g., via drag-and-drop) within specific cycles. [🤖 AI Model] [🧠 15k / 10k] [💸 $0.06 / $0.04]
- [x] `feat/task-engine-column-sort` : **List View Column Sorting** - Enable dynamic sorting by clicking on any column header within the List/Row view (Owner, Status, Timeline, Priority). [🤖 AI Model] [🧠 5k / 5k] [💸 $0.02 / $0.02]
