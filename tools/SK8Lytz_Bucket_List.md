...
- [ ] `chore/audit-sales` : Execute `@[/legacy_audit]` against `sales-module.js` to eradicate technical debt.
...
- [ ] `debt/security` : **[HIGH]** `xlsx` (SheetJS) CVEs Prototype Pollution + ReDoS. While we updated the frontend to use the patched `SheetJS (xlsx-0.20.1)` via CDN previously, the outdated NPM package `xlsx@0.18.5` is still physically defined in `package.json` dependencies and failing the security audit. It needs to be `npm uninstall xlsx` securely.
