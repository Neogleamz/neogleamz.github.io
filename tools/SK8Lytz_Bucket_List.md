...
### Target: `main`
- [ ] `bug/task-engine-shortcut-t-regression` : **Task Engine 'T' Shortcut Regression** - Fix the regression where pressing 'T' in the task engine no longer starts creating a new task under the "No Section" bucket for rapid-fire task entry. [🤖 Gemini 3.1 Pro (High)] [🧠 TBD / 5k] [💸 TBD / $0.02]
