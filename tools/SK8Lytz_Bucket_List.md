...
- [x] `debt/security` : **[HIGH] RESOLVED ✅** `xlsx` (SheetJS) CVEs Prototype Pollution + ReDoS — migrated to `exceljs@4.4.0` via CDN. `npm audit` reports 0 vulnerabilities. (commit `b2be7d0`)
...
- *(No `TODO:`, `FIXME:`, or `HACK:` markers found across all `.js`, `.html`, or `.css` files — codebase is clean.)*
