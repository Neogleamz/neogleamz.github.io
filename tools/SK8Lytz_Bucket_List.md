...
- [ ] `debt/security` : **[HIGH]** `xlsx` (SheetJS) has 2 unpatched CVEs — Prototype Pollution (`GHSA-4r6h-8v6p-xvw6`) and ReDoS (`GHSA-5pgg-2g8v-p4x9`). No fix available upstream. Evaluate replacing with a safer alternative (e.g., `exceljs`) or sandboxing all XLSX parsing behind a Supabase Edge Function to keep it off the client.
