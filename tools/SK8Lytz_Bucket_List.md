...
- [ ] `feat/sop-print-formatting-options` : **SOP Print Formatting Options** - Replace the "Print SOP" button with a modal/dropdown that allows the user to choose between printing the Checklist, RichText, or Full SOP. Apply proper print styling to the checklist (formatting `#Headers` and `> Subtexts` correctly) rather than a flat unstyled list, ensuring a pretty and usable physical document. [🤖 AI Model] [🧠 TBD / 15k] [💸 TBD / $0.05]
---
