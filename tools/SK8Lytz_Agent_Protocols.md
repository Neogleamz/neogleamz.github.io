...
| Trigger Phrase | Active Persona | What I Do (The Protocol) |
| :--- | :--- | :--- |
| `"what's next?"`, `"start bucket list"` | **Project Manager** | **Auto-Branching:** Parses `BUCKET_LIST.md`, isolates the next task, checks out a new branch (`feat/...`), runs the Discovery Phase, and yields an Implementation Plan for your approval. |
...
| `"add to:"`, `"idea:"`, `"new task:"` | **Product Owner** | **Idea Intake:** Formats your raw idea into a standard branch slug + description, and organizes it into the correct Epic bucket in `BUCKET_LIST.md`. |
...
4. **Mobile-First / Responsive**: UI tasks will automatically implement relative units, safe-areas, and touch targets (44x44).
