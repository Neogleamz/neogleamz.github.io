...
- `label_templates`: Tracks saved visual label designer templates. Fields: `id` (PK), `name`, `widthIn`, `heightIn`, `elements` (JSONB), `paper_profile` (TEXT), `created_at`.
...
- **Scanner coverage (`scripts/xss-audit.js`)**: `collectFiles()` scans `index.html` + `assets/js/*.js` **and `tools/*.html`** (extended 2026-07-20 after a stored-XSS bug in `tools/remote-scanner.html` went undetected because that file family was previously unscanned). An `ADVISORY_ONLY_PREFIXES` tier (currently `['tools' + path.sep]`) demotes findings under `tools/` to non-blocking — they still print (tagged `[ADVISORY-ONLY — does not block]`) but don't fail the pre-commit gate, so a pre-existing 15-finding inline-handler backlog in `tools/*.html` doesn't retroactively break CI. `index.html`/`assets/js/*.js` remain fully blocking with a zero-violations guarantee.
- **Known scanner blind spot**: `hasDynamic()` only recognizes `${...}` template-literal interpolation on the **same line** as the sink call (`.innerHTML =`, `document.write(...)`, etc.). A bare-variable assignment (`x.innerHTML = someVar;`, or a `document.write(html)` where `html` was assembled many lines earlier) is structurally invisible to the scanner regardless of file coverage — confirmed as the root cause behind 3 separate Critical/Moderate stored-XSS bugs (`fix/remote-scanner-history-xss`, `fix/print-reorder-report-document-write-xss`, and its `kpi-reports-module.js` sibling) that all showed 0 scanner findings both before and after their fixes. Manual code review of any `innerHTML`/`document.write`/`insertAdjacentHTML` sink is still required — do not trust a clean scanner run alone as proof of safety.
...
* click_window_openPrintSOP_currentPri was ambiguous — the 3D Layerz print handler is now **click_window_openLayerzPrintSOP_currentPri** (calls window.openSopPrintModal('production', cleanName) — not window.openPrintSOP, a prior doc-drift; window.openPrintSOP itself was deleted as dead code in `debt/nomenclature-remediation` Batch 10, and the click_window_openPrintSOP_currentPri case token was deleted in the same batch as a duplicate fallthrough label).
...
1. **Dynamic Metrics Grid**: The Physical Audit card renders a unified 6-column glassmorphism layout for all item classifications (including finished good recipes, sub-assemblies, and catalog raw goods). This provides granular, synchronized visibility into **Purchased / Produced**, **Proto Consumed**, **Prod Consumed**, **Total Consumed**, **Scrapped**, and **Expected Stock**, with all metric boxes mapped to their canonical STOCKZ hex colors (Prototype is purple `#8b5cf6`, Production is blue `#3b82f6`, Consumed is red `#ef4444`, Scrap is maroon `#b91c1c`, Expected Stock is amber/orange `#f59e0b`) to maintain absolute visual and mathematical consistency.
...
4. **Prototype Print Queue Accounting**: Print jobs spawned from a parent Prototype Work Order inherit the `wip_state.batch_type = 'Prototype'` tag. When finalized, `executePrintInventoryMath` dynamically registers the printed parts under the Prototype Produced pool (`prototype_produced_qty`) and components under the Prototype Consumed pool (`prototype_consumed_qty`), preventing prototype print metrics from misaligning the standard Production inventories.
...
* **Programmatic Git Hook Guardrail:** Enforced via [.githooks/pre-commit](file:///d:/GitHub/neogleamz.github.io/.githooks/pre-commit) (active via `git config core.hooksPath = .githooks`), which automatically runs during any `git commit` and executes three sequential gates: (1) **Root whitelist check** — parses staged changes; any unwhitelisted root-level file aborts the commit immediately. (2) **Blocking XSS audit** — runs `node scripts/xss-audit.js` (no `--warn`); promoted from advisory to blocking mode on 2026-07-01 after the codebase reached 0 violations, so any new forbidden DOM-injection pattern (UNGUARDED_INNERHTML, FORBIDDEN_TERNARY, inline handlers, unsanitized `document.write`) aborts the commit outright. (3) **Automated version bump** — runs `npm run version:bump` and auto-stages [assets/js/system-version.js](file:///d:/GitHub/neogleamz.github.io/assets/js/system-version.js) + [index.html](file:///d:/GitHub/neogleamz.github.io/index.html) cache-busters into the same commit.
